...
void RockSolidGui::setGrid(int gridSize)
{
    ...
    VisualEntity ve = staticEntity(builder, {0, 0}, CreateGrid(builder.entitySize.x, 630, 630), 2);
    gem_.visualEntityMap.insert({-1, std::move(ve)});
...
void RockSolidGui::onAddEntity(const Coordinate& pos, const GridEntity& entity) 
{
    ...
    {
        builder.instructionLayer(1).instructionTexture(it->second).addInstruction();
    }
    ve = builder.build();
...
void RockSolidGui::onReplaceEntity(const Coordinate& pos, const GridEntity& replaced, const GridEntity& added) 
{
    ...
    if (it != manager.textures.end()) 
    {
        ve.renderInstructions.push_back(RenderInstruction(std::make_unique<StaticTextureStrategy>(it->second), 1));
    }
}
