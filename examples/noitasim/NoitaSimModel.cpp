...
void NoitaSimModel::play(int column)
{
...
void NoitaSimModel::preview(int column)
{
    if (gameOver)
    {
        return;
...
void NoitaSimModel::add(int column)
{
    board[column].push(player);
    for (auto& emit : addNoitaSimEntity)
    {
...
void NoitaSimModel::remove(int column)
{
    for (auto& emit : removeNoitaSimEntity)
    {
...
void NoitaSimModel::change(const Coordinate& position, int color)
{
    for (auto& emit : changeNoitaSimEntity)
    {
        emit(position, color);
    }
}
...
std::optional<std::vector<Coordinate>> NoitaSimModel::checkWin(const std::vector<std::stack<int>>& board, int player) {
    int rows = 6;
    int cols = 7;
